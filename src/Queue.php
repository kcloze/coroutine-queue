...
use Coroutine\Queue\PushEvent;
use Coroutine\Queue\ExecEvent;
use Coroutine\Queue\Serializers\PhpSerializer;
use Coroutine\Queue\Serializers\SerializerInterface;
...
abstract class Queue 
{
    ...
    {
        parent::init();
        /** @var \Coroutine\Queue\Serializers\PhpSerializer $em */
        $this->serializer = ApplicationContext::getBean('PhpSerializer');
    }
    ...
    public function push($job)
    {
        ...
        App::trigger(self::EVENT_BEFORE_PUSH, $event);
        if ($event->handled) {
            return null;
        }
        ...
        App::trigger(self::EVENT_AFTER_PUSH, $event);
    ...
    protected function handleMessage($id, $message, $ttr, $attempt)
    {
        ...
        App::trigger(self::EVENT_BEFORE_EXEC, $event);
        if ($event->handled) {
            return true;
        }
        ...
        App::trigger(self::EVENT_AFTER_EXEC, $event);
    ...
    public function handleError($id, $job, $ttr, $attempt, $error)
    {
        ...
        App::trigger(self::EVENT_AFTER_ERROR, $event);
